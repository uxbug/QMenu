...
- (void)resourceConstrainedReadWithMaxCount:(int)max success:(AssetCatalogReaderSuccessHandler)success failure:(AssetCatalogReaderFailureHandler)failure {
    self.resourceConstrained = YES;
    self.maxCount = max;
    
    [self readWithProgress:nil success:success failure:failure];
}

- (void)readWithProgress:(AssetCatalogReaderProgressHandler)progress success:(AssetCatalogReaderSuccessHandler)success failure:(AssetCatalogReaderFailureHandler)failure {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        if (!catalogPath) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSError *error = [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"Unable to find asset catalog path"}];
        ...
        if (self.distinguishCatalogsFromThemeStores) {
            if (!self.catalog.allImageNames.count || ![self.catalog respondsToSelector:@selector(imageWithName:scaleFactor:)]) {
                ...
                return [self readThemeStoreWithProgress:progress success:success failure:failure];
            }
        } else {
            return [self readThemeStoreWithProgress:progress success:success failure:failure];
        }
        ...
        for (NSString *imageName in self.catalog.allImageNames) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (progress) progress(loadedFraction);
            });
        ...
        if (!self.images.count) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSError *error = [NSError errorWithDomain:kAssetCatalogReaderErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey: @"Failed to load images"}];
                !failure ?: failure(error);
            });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            !success ?: success(self.images);
        });
    });
}


- (void)readThemeStoreWithProgress:(AssetCatalogReaderProgressHandler)progress success:(AssetCatalogReaderSuccessHandler)success failure:(AssetCatalogReaderFailureHandler)failure {
    ...
    [[self.catalog _themeStore].themeStore.allAssetKeys enumerateObjectsWithOptions:0 usingBlock:^(CUIRenditionKey * _Nonnull key, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            double loadedFraction = (double)loadedItemCount / (double)totalItemCount;
            !progress ?: progress(loadedFraction);
        });
        
        @try {
            CUIThemeRendition *rendition = [[weakSelf.catalog _themeStore] renditionWithKey:key.keyList];
            
            // when resource constrained and the catalog contains retina images, only load retina images
            if ([weakSelf catalogHasRetinaContent] && weakSelf.resourceConstrained && rendition.scale < 2) {
                return;
            }
            
            NSString *filename = [weakSelf filenameForAssetNamed:[weakSelf cleanupRenditionName:rendition.name] scale:rendition.scale presentationState:key.themeState];
            ...
            if (rendition.unslicedImage) {
                ...
                NSDictionary *desc = [weakSelf imageDescriptionWithName:rendition.name filename:filename representation:imageRep];
                
                BOOL ignore = [filename containsString:@"ZZPackedAsset"] && weakSelf.ignorePackedAssets;
                ...
                [weakSelf.mutableImages addObject:desc];
            } else {
